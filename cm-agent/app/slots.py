...
def channel_rules(brand_id, channels_list):
    """[(channel, window_start, window_end, min_gap_min)] z channels.config."""
    rows = db.fetchall(
    ...
    for r in rows:
        ...
        out.append((r["channel"], ws, we, gap))
    return out
...
def next_slot(brand_id, channels_list, is_article=False, prefer_today=True):
    ...
    for d in range(LOOKAHEAD_DAYS):
        ...
        ws = max(w for _, w, _, _ in rules)
        we = min(w for _, _, w, _ in rules)
        if ws >= we:
            continue
