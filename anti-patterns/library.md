...
| **307** | Nowy kontrakt zbudowany bez przelaczenia ZYWEGO konsumenta starego - cala maszyneria slotow dzialala, a `publish_mode='webhook'` omijal ja w calosci i wystrzelil piec postow w godzine. | [AP-307](../docs/anti-patterns/AP-307_nowy_kontrakt_bez_przelaczenia_konsumenta.md) |
...
**AP-316 stoi osobno i jest o DOKUMENTACJI, nie o kodzie** - ale nalezy do tej samej rodziny
co 307 i 309, tylko o warstwe wyzej: poprawka w kodzie nie propaguje sie do instrukcji,
bo instrukcji nikt nie kompiluje.
...
pracy z systemem. Sa nizej w tym pliku, z wlasnymi pelnymi opisami. Jedenastka wyzej jest
...
**Correct:** when a build changes a contract (slots become meaningful, language becomes per-channel), enumerate EVERY live path consuming that contract (here: publish_mode per channel + publisher callbacks) and switch or verify each IN THE SAME BUILD; end with an end-to-end probe through the path that will actually run in production, not the path you just wrote. **Dopisane 11/08:** zapisany anty-wzorzec nie jest wdrozonym anty-wzorcem - DEPLOY_CHECKLIST przez trzy tygodnie po incydencie nadal instruowal instalatora, zeby ustawil `publish_mode='webhook'`, czyli kazal odtworzyc te awarie u nowego klienta. Po zapisaniu anty-wzorca sprawdz, czy jakikolwiek dokument INSTRUKTAZOWY nadal uczy starego zachowania: kod poprawiasz, bo pamietasz gdzie mieszka, a instrukcje sa gdzie indziej i nikt ich nie kompiluje. Pelny opis: docs/anti-patterns/AP-307_nowy_kontrakt_bez_przelaczenia_konsumenta.md.
